...
export const GalleryCard = ({ paramSearch }) => {
  ...
  const { data, isLoading, errors} = useFetch(url,{name: 'agumon'}) 
  ...
  return (
    ...
    {
      isLoading 
        ? (<div className="absolute -translate-x-2/4 left-2/4 top-40 text-4xl">Cargando...</div>)
        : errors ? (
          <div className="text-red-500 absolute -translate-x-2/4 left-2/4 top-40 text-xl">Hubo un error al cargar los datos. Por favor, inténtalo de nuevo más tarde.</div>
        )  
